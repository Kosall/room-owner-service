...
public class OutboxWorker {
	...
	public void drainOutbox() {
		repo.findByStatusAndAvailableAtLessThanEqualOrderByOccurredAtAsc(OutboxStatus.PENDING, LocalDateTime.now(),
				PageRequest.of(0, props.getMaxBatch())).flatMap(this::tryPublish, 1) // sequential to preserve ordering
																						// per key; adjust if you shard
				.onErrorContinue((ex, obj) -> log.error("Outbox worker error: {}", ex.getMessage(), ex)).subscribe();
	}
	...
	private Mono<OutboxEvent> tryPublish(final OutboxEvent e) {
		...
		return publisher.publish(e.getTopic(), e.getKey(), envelope).then(markSent(e))
